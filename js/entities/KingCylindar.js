                    ...
                    for (const orb of orbs) {
                        ...
                        for (let i = 4; i < 7 && i < orb.children.length; i++) {
                            ...
                            const rotationMatrix = new THREE.Matrix4().makeRotationAxis(
                                axis, 
                                ring.userData.rotationSpeed * 0.016
                            );
                            ...
                            // Update ring color
                            if (ring.material) {
                                const hue = (orb.userData.colorCycle + ((i-4) * 0.33)) % 1;
                                const color = new THREE.Color().setHSL(hue, 0.8, 0.6);
                        ...
                        // Update lights
                        // Core light - amber/gold pulsing
                        const coreLight = orb.children[7]; // First light
                        if (coreLight) {
                            const hue = (orb.userData.colorCycle) % 1;
                            const color = new THREE.Color().setHSL(hue, 0.8, 0.6);
                            coreLight.color.set(color);
                            coreLight.intensity = 2.0 + Math.sin(time * 1.5) * 0.5;
                        }
                        
                        // Secondary lights with color cycling
                        if (orb.children[8]) { // Second light
                            const hue1 = (orb.userData.colorCycle + 0.33) % 1;
                            const color1 = new THREE.Color().setHSL(hue1, 0.8, 0.6);
                            orb.children[8].color.set(color1);
                            orb.children[8].intensity = 1.5 + Math.sin(time * 2) * 0.5;
                        }
                        
                        if (orb.children[9]) { // Third light
                            const hue2 = (orb.userData.colorCycle + 0.66) % 1;
                            const color2 = new THREE.Color().setHSL(hue2, 0.8, 0.6);
                            orb.children[9].color.set(color2);
                            orb.children[9].intensity = 1.0 + Math.sin(time * 2.5) * 0.5;
                        }
                    }
                }
                
                requestAnimationFrame(animatePsychedelicOrbs);
            };
            
            animatePsychedelicOrbs();
        }
        
        // Add to animated orbs
        window.animatedPsychedelicOrbs.push(orbMesh);
    }
