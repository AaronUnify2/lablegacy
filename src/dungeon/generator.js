...
function createCorridor(dungeon, room1, room2) {
    ...
    if (Math.random() < 0.5) {
        // Horizontal then vertical
        createHorizontalCorridor(dungeon, centerX1, centerX2, centerZ1);
        createVerticalCorridor(dungeon, centerZ1, centerZ2, centerX2);
    } else {
        // Vertical then horizontal
        createVerticalCorridor(dungeon, centerZ1, centerZ2, centerX1);
        createHorizontalCorridor(dungeon, centerX1, centerX2, centerZ2);
    }
}

// Create a horizontal corridor segment
function createHorizontalCorridor(dungeon, startX, endX, z) {
    const corridorWidth = 3; // Width of the corridor
    const minX = Math.min(startX, endX);
    const maxX = Math.max(startX, endX);
...
// Create a vertical corridor segment
function createVerticalCorridor(dungeon, startZ, endZ, x) {
    const corridorWidth = 3; // Width of the corridor
    const minZ = Math.min(startZ, endZ);
    const maxZ = Math.max(startZ, endZ);
...
function placeKeyAndExit(dungeon) {
    ...
    dungeon.setPlayerSpawnPosition(
        spawnRoom.x + spawnRoom.width / 2,
        0.5, // Slight elevation from floor
        spawnRoom.z + spawnRoom.height / 2
    );
}
...
function addDecorations(dungeon, theme) {
    ...
    for (const room of rooms) {
        ...
        for (let i = 0; i < decorationCount; i++) {
            ...
            // Add decoration based on theme
            dungeon.addDecoration(x, 0, z, theme);
        }
    }
}
